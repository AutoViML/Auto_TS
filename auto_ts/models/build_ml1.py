...
class BuildML(BuildBase):
    ...
    def predict(
        ...
        if self.univariate:
            ...
            testdata_with_dummy = copy.deepcopy(testdata)
            testdata_with_dummy[self.original_target_col] = 0
            ...
            if  self.df_train_prepend.shape[1] == testdata_with_dummy.shape[1]:
                copy_cols1 = self.df_train_prepend.columns.tolist()
                copy_cols2 = testdata_with_dummy.columns.tolist()
                leftover = left_subtract(copy_cols1, copy_cols2)
                if len(leftover):
                    ### This means that the columns in test and train_prepend don't match! ###
                    print('Alert! Extra columns in train data compared to test data. Taking train_data as ground truth...')
                    try:
                        testdata_with_dummy = testdata_with_dummy[copy_cols1]
                    except:
                        print('Error: columns mismatch between train and test data. Please fix it and rerun.')
                        return
                ## This means that the columns in test and train_prepend exactly match ###
                df_prepend = pd.concat([self.df_train_prepend, testdata_with_dummy], axis=0)
                    
            #### This is where you change the model to dask if dask_xgboost flag is set #####
            ...
            ### Once the test and train_prepend are appended, then they are ready to process row by row ##
            for i in np.arange(self.lags, df_prepend.shape[0]):
                ### first we need to convert the dataframe into a supervised ML dataset ##
                dfxs, _, _  = self.df_to_supervised_test(ts_df=df_prepend, drop_zero_var=False)

                # Select only the predictors (transformed) from here
                X_test = dfxs[self.transformed_preds].iloc[:i+1,:]
            
                ######## Now make forecast one row at a time ########            
                if isinstance(testdata, pd.DataFrame) or isinstance(testdata, pd.Series):
                    if self.dask_xgboost_flag:
